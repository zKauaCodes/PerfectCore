...
import eu.perfect.core.menu.calculateEndLine
import eu.perfect.core.menu.calculateSlot
import eu.perfect.core.menu.rawSlot
...
interface MenuPlayerInventory : MenuPlayer {
    ...
    fun getPlayerItem(slot: Int): ItemStack? = player.inventory.getItem(
        rawSlot(
            slot
        )
    )
    ...
    private fun getLineRange(line: Int) = calculateSlot(
        line,
        1
    )..calculateEndLine(line)
...
interface MenuPlayerMove : MenuPlayerInventory,
    MenuPlayerCancellable {
    val toMoveSlot: Int
...
class MenuPlayerMoveTo(
    ...
    var targetSlot: Int
) : MenuPlayerInteract(menu, player, inventory, canceled),
    MenuPlayerMove {
